...
public class QuestionServiceImpl extends ServiceImpl<QuestionMapper, Question> {
    ...
    public PageWrapper<Question> getQuestionPage(QuestionPageRequest request) {
        ...
        // 内容
        lqw.eq(StringUtils.isNotBlank(request.getContent()), Question::getContent, request.getContent());
        // 参考答案
        lqw.eq(StringUtils.isNotBlank(request.getRefAnswer()), Question::getRefAnswer, request.getRefAnswer());
        // 创建来源
        lqw.eq(StringUtils.isNotBlank(request.getCreateSource()), Question::getCreateSource, request.getCreateSource());
        // 用户ID
        lqw.eq(StringUtils.isNotBlank(request.getUserId()), Question::getUserId, request.getUserId());
        // 题目等级
        lqw.eq(StringUtils.isNotBlank(request.getQuestionLevel()), Question::getQuestionLevel, request.getQuestionLevel());
        // 标签
        ...
        // 提交答案量
        lqw.eq(request.getCommitAnswerCnt() != null, Question::getCommitAnswerCnt, request.getCommitAnswerCnt());
        // 收藏量
        lqw.eq(request.getFavCnt() != null, Question::getFavCnt, request.getFavCnt());
        // 扩展信息
        lqw.eq(StringUtils.isNotBlank(request.getExtendInfo()), Question::getExtendInfo, request.getExtendInfo());
        // 创建人
        lqw.eq(StringUtils.isNotBlank(request.getCreateBy()), Question::getCreateBy, request.getCreateBy());
        // 创建时间
        lqw.eq(request.getCreateTime() != null, Question::getCreateTime, request.getCreateTime());
        // 创建时间 start
        lqw.ge(request.getCreateTimeStart() != null, Question::getCreateTime, request.getCreateTimeStart());
        // 创建时间 end
        lqw.le(request.getCreateTimeEnd() != null, Question::getCreateTime, request.getCreateTimeEnd());
        // 修改人
        lqw.eq(StringUtils.isNotBlank(request.getUpdateBy()), Question::getUpdateBy, request.getUpdateBy());
        // 修改时间
